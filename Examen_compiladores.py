...
def analizar_codigo():
    ...
    for tok in tokens:
        ...
        result_tree.insert("", 'end', values=(
            tok.value, palabra_reservada, identificador, operador, numero, simbolo, tok.lineno))


ventana = tk.Tk()
ventana.title("Examen Analizador Léxico")

frame_izquierda = ttk.Frame(ventana)
frame_izquierda.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

entrada_codigo = ScrolledText(
    frame_izquierda, width=40, height=10, wrap=tk.WORD)
entrada_codigo.pack(fill="both", expand=True)

boton_analizar = tk.Button(
    frame_izquierda, text="Analizar Código", command=analizar_codigo)
boton_analizar.pack(pady=5)

frame_derecha = ttk.Frame(ventana)
frame_derecha.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")

result_tree = ttk.Treeview(frame_derecha, columns=("Token", "Palabra Reservada",
                           "Identificador", "Operador", "Número", "Símbolo", "Línea"), show="headings")
...
result_tree.column("Token", width=70)
result_tree.column("Palabra Reservada", width=120)
result_tree.column("Identificador", width=120)
result_tree.column("Operador", width=70)
result_tree.column("Número", width=70)
result_tree.column("Símbolo", width=70)
result_tree.column("Línea", width=70)
